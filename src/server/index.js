...
app.use(function(req, res, next) {
	res.ssr = function(options) {
		...
		if (options.disable) {
			page = page.replace(/<div[^>]*>.*?(?=<\/script)/i, '<div>')
			page = page.replace(/<script type="__PREACT_CLI_DATA__[^>]*>.*?(<\/script>)/i, 'test')
		} else {
			let html = renderToString(h(bundle.default, { CLI_DATA: { preRenderData: options.props }}));

			html += '<script type="__PREACT_CLI_DATA__">' + encodeURI(JSON.stringify({
